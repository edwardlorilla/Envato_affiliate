...
(function ($) {
    ...
    $.extend(File.prototype, {
        ...
        value2input: function(value) {
            if(!value) {
				this.$input.filter('[name="delete"]').parent().css('visibility', 'hidden');
                return;
            }
	    	...
	    	this.$input.filter('[name="delete"]').parent().css('visibility', 'visible');
        },
        ...
        input2value: function() {
			var f = this.$input.filter('[name="file"]').val();
			if (this.$input.filter('[name=delete]').is(':checked')) {
				f = '@delete';
			}
            return {
                file: f
    ...
    File.defaults = $.extend({}, $.fn.editabletypes.abstractinput.defaults, {
        tpl: '<div class="editable-file">' +
            ...
            '<button type="button" style="cursor: pointer; display: inline-block; margin-right: 5px;">Choose file</button>' +
            '</span>' +
	    '<span style="vertical-align: top; visibility: hidden;"><input type="checkbox" value="1" name="delete" /> Delete</span>' +
	    '</div>',
